...
def main():
    ...
    parser = argparse.ArgumentParser(description="Migrate TOML configuration to API database")
    ...
    if not api_endpoint or not api_key:
        print("Error: API_ENDPOINT and API_KEY must be provided as arguments or environment variables")
        sys.exit(1)
    ...
    if "bank-account-map" not in config_data or "expense-account-map" not in config_data:
        print("Error: Configuration must contain both 'bank-account-map' and 'expense-account-map' sections")
        sys.exit(1)
    ...
    print(f"Will import {len(import_data['expense-account-map'])} expense account mappings")
