...
class TestCheckConfigs:
    ...
    @patch('builtins.open', new_callable=mock_open, read_data='{"DATABASE_URL": "postgresql://test", "REDIS_URL": "redis://test"}')
    ...
    @patch.dict(os.environ, {'DATABASE_URL': 'postgresql://env_test', 'REDIS_URL': 'redis://env_test'})
    ...
    def test_invalid_configuration_handling(self):
        ...
        for config in invalid_configs:
            ...
            if "polling_interval" in config:
                assert config["polling_interval"] not in ["hourly", "daily"] or \
                       not isinstance(config.get("is_enabled"), bool) or \
                       (isinstance(config.get("imap_port"), int) and config.get("imap_port", 0) <= 0) or \
                       len(config) == 0 