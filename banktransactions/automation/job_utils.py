    ...
    try:
        ...
        for job_id in started_registry.get_job_ids():
            try:
                ...
                if (job.func_name == 'banktransactions.email_automation.workers.email_processor.process_user_emails_standalone' and 
                    len(job.args) > 0 and 
                    job.args[0] == user_id):
...
def is_user_job_queued(redis_conn, user_id: int, queue_name: str = "email_processing") -> bool:
    """
...
def has_user_job_pending(redis_conn, user_id: int, queue_name: str = "email_processing") -> bool:
    """
    ...
    """
    return (is_user_job_running(redis_conn, user_id) or 
            is_user_job_scheduled(redis_conn, user_id) or 
            is_user_job_queued(redis_conn, user_id, queue_name))


def get_user_job_status(redis_conn, user_id: int, queue_name: str = "email_processing") -> dict:
    """
    ...
    status["has_any_pending"] = (status["has_running_job"] or 
                                status["has_scheduled_job"] or 
                                status["has_queued_job"])
