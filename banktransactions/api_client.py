...
def send_transaction_to_api(transaction_data):
    ...
    api_endpoint = os.getenv("API_ENDPOINT") # Should be like http://host/api/v1/transactions
    ...
    try:
        ...
        logging.error(f"Invalid amount format: {transaction_data.get('amount')}. Skipping API call")
        return False
    ...
    if raw_date:
        try:
            ...
            logging.error(f"Invalid or unparseable date format: {raw_date}. Error: {e}. Skipping API call")
            return False
    else:
