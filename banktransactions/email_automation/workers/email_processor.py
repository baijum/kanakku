...
from email_parser import extract_transaction_details_pure_llm, detect_currency, convert_currency
...
class EmailProcessor:
    ...
    def process_user_emails(self, user_id: int) -> Dict:
        ...
        try:
            ...
            config = self.db.query(EmailConfiguration).filter_by(user_id=user_id).first()
            if not config or not config.is_enabled:
                return {"status": "skipped", "reason": "configuration_not_found_or_disabled"}
                ...
                try:
                    ...
                    if transaction_data and transaction_data.get('amount') != "Unknown":
                        ...
                        if response.get('success'):
                            ...
                            logger.info(f"Successfully processed email {email['id']} for user {user_id}")
                        else:
                            error_msg = f"Failed to create transaction for email {email['id']}: {response.get('error', 'Unknown error')}"
                            logger.error(error_msg)
                            errors.append(error_msg)
                    else:
                        logger.info(f"Skipping email {email['id']} - no valid transaction data found")
