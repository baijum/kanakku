...
class TestSchedulerDeduplication:
    ...
    def test_scheduler_skips_when_job_pending(self, email_scheduler, mock_email_config):
        ...
        with patch('banktransactions.email_automation.workers.scheduler.has_user_job_pending') as mock_has_pending:
            with patch('banktransactions.email_automation.workers.scheduler.get_user_job_status') as mock_get_status:
                ...
                with patch.object(email_scheduler, '_calculate_next_run') as mock_calc_next:
                    ...
                    mock_has_pending.assert_called_once_with(email_scheduler.redis_conn, 123)
                    
                    # Verify that scheduler.enqueue_at was NOT called
                    email_scheduler.scheduler.enqueue_at.assert_not_called()

    def test_scheduler_proceeds_when_no_job_pending(self, email_scheduler, mock_email_config):
        """Test that scheduler proceeds with scheduling when no job is pending."""
        
        # Mock has_user_job_pending to return False
        with patch('banktransactions.email_automation.workers.scheduler.has_user_job_pending') as mock_has_pending:
            with patch('banktransactions.email_automation.workers.scheduler.generate_job_id') as mock_gen_id:
    ...
    def test_schedule_jobs_with_multiple_users(self, email_scheduler):
        ...
        # Mock database query
        email_scheduler.db.query.return_value.filter_by.return_value.all.return_value = configs
    ...
    def test_error_handling_in_schedule_user_job(self, email_scheduler, mock_email_config):
