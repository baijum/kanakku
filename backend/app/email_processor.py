...
def process_emails():
    ...
    if not gemini_api_token:
        current_app.logger.error("Gemini API token not configured. Email processing aborted.")
        return False
    
    # Log that we have the token (don't log the actual token value)
    current_app.logger.info("Gemini API token retrieved successfully from global configuration.")
    
    # Placeholder for email processing
    current_app.logger.info("Email processing would occur here in a future implementation.")
...
def analyze_email_with_gemini(email_content):
    ...
    if not gemini_api_token:
        current_app.logger.error("Gemini API token not configured. Cannot analyze email.")
        return None
