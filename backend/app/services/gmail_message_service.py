...
from ..models import ProcessedGmailMessage, User
from ..extensions import db

logger = logging.getLogger(__name__)


def load_processed_gmail_msgids(user_id: int, db_session: Optional[Session] = None) -> Set[str]:
    """
    ...
    try:
        ...
        processed_messages = session.query(ProcessedGmailMessage.gmail_message_id).filter_by(
            user_id=user_id
        ).all()
        ...
        logger.info(f"Loaded {len(msgids)} processed Gmail Message IDs for user {user_id}")
        logger.debug(f"Processed message IDs for user {user_id}: {list(msgids)[:10]}...")  # Log first 10 for debugging
        
        return msgids
        
    except Exception as e:
        logger.error(f"Error loading processed Gmail Message IDs for user {user_id}: {e}")
        logger.debug(f"Exception details: {type(e).__name__}: {str(e)}")
        return set()  # Return empty set on error


def save_processed_gmail_msgid(user_id: int, gmail_message_id: str, db_session: Optional[Session] = None) -> bool:
    """
    ...
    try:
        ...
        logger.debug(f"Saved processed Gmail Message ID {gmail_message_id} for user {user_id}")
        return True
        
    except IntegrityError as e:
        # Handle duplicate entries gracefully
        session.rollback()
        logger.debug(f"Gmail Message ID {gmail_message_id} already exists for user {user_id}")
        return True  # Consider it successful since the message is already marked as processed
        
    except Exception as e:
        session.rollback()
        logger.error(f"Error saving processed Gmail Message ID {gmail_message_id} for user {user_id}: {e}")
        logger.debug(f"Exception details: {type(e).__name__}: {str(e)}")
        return False


def save_processed_gmail_msgids(user_id: int, msgids: Set[str], db_session: Optional[Session] = None) -> int:
    """
    ...
    try:
        ...
        if not new_msgids:
            logger.debug(f"All {len(msgids)} Gmail Message IDs already exist for user {user_id}")
            return 0
        ...
        logger.info(f"Saved {len(new_records)} new processed Gmail Message IDs for user {user_id}")
        return len(new_records)
        
    except Exception as e:
        session.rollback()
        logger.error(f"Error saving processed Gmail Message IDs for user {user_id}: {e}")
        logger.debug(f"Exception details: {type(e).__name__}: {str(e)}")
        return 0


def is_gmail_message_processed(user_id: int, gmail_message_id: str, db_session: Optional[Session] = None) -> bool:
    """
    ...
    try:
        ...
        exists = session.query(ProcessedGmailMessage).filter_by(
            user_id=user_id,
            gmail_message_id=gmail_message_id
        ).first() is not None
        
        logger.debug(f"Gmail Message ID {gmail_message_id} processed status for user {user_id}: {exists}")
        return exists
        
    except Exception as e:
        logger.error(f"Error checking processed status for Gmail Message ID {gmail_message_id} and user {user_id}: {e}")
        logger.debug(f"Exception details: {type(e).__name__}: {str(e)}")
        return False  # Assume not processed on error to avoid skipping messages


def get_processed_message_count(user_id: int, db_session: Optional[Session] = None) -> int:
    """
...
def clear_processed_gmail_msgids(user_id: int, db_session: Optional[Session] = None) -> bool:
    """
    ...
    try:
        ...
        deleted_count = session.query(ProcessedGmailMessage).filter_by(user_id=user_id).delete()
        session.commit()
        
        logger.info(f"Cleared {deleted_count} processed Gmail Message IDs for user {user_id}")
        return True
        
    except Exception as e:
        session.rollback()
        logger.error(f"Error clearing processed Gmail Message IDs for user {user_id}: {e}")
        logger.debug(f"Exception details: {type(e).__name__}: {str(e)}")
        return False 