...
def demo_gemini_api_operations():
    ...
    if is_configured:
        ...
        print(f"   Current token (first 10 chars): {current_token[:10] if current_token else 'None'}...")
    ...
    if success:
        ...
        print(f"   Retrieved token matches: {'✅ Yes' if retrieved_token == demo_token else '❌ No'}")
...
def demo_list_all_configurations():
    ...
    if all_configs:
        ...
        for config in all_configs:
            ...
            print(f"   {encrypted_indicator} {config['key']}: {config['value']} ({config.get('description', 'No description')})")
    else:
...
def main():
    ...
    with app.app_context():
        try:
            ...
            print("- Sensitive values are automatically encrypted when is_encrypted=True")
            print("- Use specialized functions like set_gemini_api_token() for validated settings")
