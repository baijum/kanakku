...
class TestGmailMessageService:
    ...
    def test_save_processed_gmail_msgid_success(self, app, db_session, user):
        ...
        # Verify it was saved
        saved_msg = db_session.query(ProcessedGmailMessage).filter_by(
            user_id=user.id, gmail_message_id="msg_789"
        ).first()
    ...
    def test_save_processed_gmail_msgids_new_messages(self, app, db_session, user):
        ...
        saved_count = db_session.query(ProcessedGmailMessage).filter_by(
            user_id=user.id
        ).count()
        assert saved_count == 3

    def test_save_processed_gmail_msgids_mixed_new_existing(self, app, db_session, user):
    ...
    def test_save_processed_gmail_msgids_database_error(self, app, user):
        ...
        with app.app_context():
            with patch('app.services.gmail_message_service.load_processed_gmail_msgids') as mock_load:
    ...
    def test_clear_processed_gmail_msgids_success(self, app, db_session, user):
        ...
        remaining_count = db_session.query(ProcessedGmailMessage).filter_by(
            user_id=user.id
        ).count()
