...
def test_create_preamble(authenticated_client, user):
    ...
    response = authenticated_client.post('/api/v1/preambles', json={
        'name': 'New Preamble',
        'content': 'New content',
        'is_default': True
    })
...
def test_update_preamble(authenticated_client, user, app):
    ...
    # Test partial update
    response = authenticated_client.put(f'/api/v1/preambles/{preamble_id}', json={
        'name': 'Partially Updated Preamble'
    })
    assert response.status_code == 200
    data = response.get_json()
    ...
    response = authenticated_client.put('/api/v1/preambles/999999', json={
        'name': 'Non-existent Preamble'
    })
    assert response.status_code == 404
