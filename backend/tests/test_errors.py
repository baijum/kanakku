...
def test_test_error_endpoint(app, client, monkeypatch):
    ...
    assert any("Test error endpoint called" in str(args) for args in mock_logger.info.call_args_list)
...
def test_test_db_error_endpoint(app, client, monkeypatch):
    ...
    # Verify info logger was called for the endpoint
    assert any("Test database error endpoint called" in str(args) for args in mock_logger.info.call_args_list)
