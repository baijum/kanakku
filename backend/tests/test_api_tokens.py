...
def api_token(app, db_session, user):
    ...
    existing_token = (
        ...
        .filter_by(user_id=user.id, name="Test Token")
        .first()
    )
    if existing_token:
...
def test_get_tokens(authenticated_client, api_token, db_session):
    ...
    for token in data:
        if token["id"] == api_token.id:
            ...
            assert "token" not in token  # Token value should not be included in the list
            break
...
def test_update_token(authenticated_client, api_token, db_session):
    ...
    response = authenticated_client.put(
        f"/api/auth/tokens/{api_token.id}",
...
def test_token_model_methods(app, db_session, user):
    ...
    with app.app_context():
        ...
        if updated_token.last_used_at.tzinfo is None:
            updated_token.last_used_at = updated_token.last_used_at.replace(tzinfo=timezone.utc)
            
        assert (now - updated_token.last_used_at).total_seconds() < 10  # Should be recent 