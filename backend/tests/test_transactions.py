...
def test_update_transaction(authenticated_client, user, app):
    ...
    response = authenticated_client.put(f'/api/transactions/{transaction_id}', json={
        'payee': 'Updated',
        'amount': 200.0
    })
    ...
    with app.app_context():
        ...
        assert updated_account.balance == 1200.0  # 1000 + 100 (initial) + 100 (difference)
...
def test_delete_related_transactions(authenticated_client, user, app):
    ...
    response = authenticated_client.delete(f'/api/transactions/{transaction_id}/related')
    assert response.status_code == 200
    data = response.get_json()
    assert data['count'] == 2
