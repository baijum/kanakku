...
def test_update_password(authenticated_client, user, app, db_session):
    ...
    response = authenticated_client.put('/api/auth/password', json={
...
def test_activate_user(authenticated_client, user, app):
    ...
    response = authenticated_client.post(f'/api/auth/users/{user.id}/activate', json={
        'is_active': True
    })
    assert response.status_code == 200
    data = response.get_json()
    ...
    response = authenticated_client.post('/api/auth/users/999999/activate', json={
        'is_active': True
    })
...
def test_google_callback_invalid_state(client, app):
    ...
    response = client.get('/api/auth/google/callback?state=invalid_state&code=test-code')
    assert response.status_code == 400
    data = response.get_json()
