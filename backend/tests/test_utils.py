    ...
    def test_log_request(self, app, monkeypatch):
        ...
        with app.test_client() as client:
            with app.test_request_context('/api/test', 
                ...
                log_data = json.loads(mock_logger.info.call_args[0][0].replace("Request: ", ""))
                ...
                log_request(include_headers=True, include_body=True, sanitize_fields=["password"])
                mock_logger.info.assert_called_once()
                log_data = json.loads(mock_logger.info.call_args[0][0].replace("Request: ", ""))
    ...
    def test_log_response(self, app, monkeypatch):
        ...
        with app.test_client() as client:
            with app.test_request_context('/api/test'):
                ...
                response.get_data.return_value = json.dumps({"status": "success", "data": "test"})
                ...
                mock_logger.info.assert_called_once()
                log_data = json.loads(mock_logger.info.call_args[0][0].replace("Response: ", ""))
...
class TestEmailUtils:
    ...
    def test_send_password_reset_email(self, mock_mail, app, monkeypatch):
        ...
        assert "Sending password reset email" in mock_logger.info.call_args_list[0][0][0]
        assert "Password reset email sent successfully" in mock_logger.info.call_args_list[1][0][0]
        ...
        mock_logger.error.assert_called_once()
        assert "Failed to send password reset email" in mock_logger.error.call_args[0][0]
