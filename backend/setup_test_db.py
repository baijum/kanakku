...
def setup_test_db():
    ...
    with app.app_context():
        ...
        # Create a test user
        user = User(
            email="admin@example.com", name="Admin User", is_admin=True, is_active=True
        )
        ...
        # Create a book
        book = Book(name="Default Book", user_id=user.id)
        db.session.add(book)
        db.session.commit()
        ...
        print("User: admin@example.com with password: password123")
